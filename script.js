...
function GenerateUsername() {
  ...
  var favoriteNumber = parseInt(document.getElementById('favNum').value);
    ...
    if (numUsernames > 5) {
      ...
      document.getElementById('generatedUsername').textContent = "Error: Number of usernames can not exceed 5";
      return; 
    }
  
  var generatedUsername = generateUsername(numUsernames, interest, favoriteNumber);
  document.getElementById('generatedUsername').textContent = generatedUsername;
  writeToFile('username.json', { generatedUsername });
  }
  ...
  function generateUsername(numUsernames, interests, favoriteNumber) {
    ...
    return usernames.join(' - '); // Join the usernames with a hyphen
}
